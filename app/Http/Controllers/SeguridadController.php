...
{    
    public function perfilusuarioobjetosel(Request $request): JsonResponse{
            ...
            try {
                ...
                $p_apl_id = $request->has('p_apl_id') ? (int) $request->input('p_apl_id') : 2;
    ...
    public function cambiarclave(Request $request)
    {
        ...
        try {
            $p_usu_id =  $request->has('p_usu_id') ? (int) $request->input('p_usu_id') : 0;
            $p_usu_pasold =  $request->has('p_usu_pasold') ? (string) $request->input('p_usu_pasold') : '';
            $p_usu_pasnew =  $request->has('p_usu_pasnew') ? (string) $request->input('p_usu_pasnew') : '';

            $results = DB::select('SELECT * FROM seguridad.spu_usuarioclave_chg(?, ?, ?)',[$p_usu_id, $p_usu_pasold, $p_usu_pasnew]);
